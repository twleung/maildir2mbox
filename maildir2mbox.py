    ...
    try:
        ...
        for i, v in enumerate(maildir.iteritems()):
            ...
            try:
                ...
                info("Exception while processing msg with key: %s" % key)
                # traceback.print_exc()
                # raise
    finally:
        # close and unlock
...
def convert(maildir_path, mbox_path, recurse):
    ...
    maildir_sub_path = [
        p
        for p in maildir_path.parent.iterdir()
    ...
    for subdir in maildir_sub_path2:
        mbox_dir_sub_path = Path(
            mbox_path.encode("utf-8")
            + subdir.name[len(mdp_prefix) - 1 :].replace(".", ".sbd/")
            + ".sbd"
        )
        mbox_sub_path = Path((mbox_dir_sub_path.encode("utf-8"))[:-4])
    ...
    mdp_prefix = maildir_path.parts[-1] + "."
    maildir_sub_path = [
        (Path(dirinfo[0]) / subdir).relative_to(maildir_path)
        for dirinfo in os.walk(str(maildir_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "maildir_path",
        help=(
            "path to the existing maildir " "(containing new, cur, tmp) subdirectories"
        ),
    )
    parser.add_argument(
        "mbox_filename",
        help=(
            "target filename in the mbox format. If the mailbox already exists, new messages are appended to it."
        ),
    )
    parser.add_argument(
        "-r",
        "--recurse",
        dest="recurse",
        help="Process all mail folders included in maildir_path. An equivalent "
